...
namespace DuiLib
{
	...
	LONG CSliderUI::GetChangeStep()
	{
		return m_nStep;
	}

	void CSliderUI::SetChangeStep(LONG step)
	{
		m_nStep = step;
	}
	...
	LRESULT CSliderUI::DoEvent(TEventUI& event)
	{
	...
	LRESULT CSliderUI::PaintStatusImage(HDC hDC)
	{
		...
		if (DrawImage(hDC, m_diThumb) >= (0L)) return (0L);
		return (0L);
	}
}
