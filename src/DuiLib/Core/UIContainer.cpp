...
namespace DuiLib
{
	...
	CControlUI* CContainerUI::GetItemAt(LONG iIndex) const
	{
		if ( iIndex < 0 || iIndex >= m_items.GetSize() ) return NULL;
		return static_cast<CControlUI*>(m_items[iIndex]);
	}

	LONG CContainerUI::GetItemIndex(CControlUI* pControl) const
	{
	...
	LRESULT CContainerUI::SetItemIndex(CControlUI* pControl, LONG iNewIndex)
	{
		for( int it = 0; it < m_items.GetSize(); it++ ) {
			if ( static_cast<CControlUI*>(m_items[it]) == pControl ) {
				NeedUpdate();            
				m_items.Remove(it);
				return m_items.InsertAt(iNewIndex, pControl) >= 0 ? (0L) : (-1L);
			}
		}
	...
	LRESULT CContainerUI::SetMultiItemIndex(CControlUI* pStartControl, LONG iCount, LONG iNewStartIndex)
    {
        ...
        for( int it3 = 0; it3 < pControls.GetSize(); it3++ ) {
            if (pControls.InsertAt(iNewStartIndex + it3, pControls[it3]) < (0L)) return (-1L);
        }
	...
	LONG CContainerUI::GetCount() const
	{
		return m_items.GetSize();
	}

	LRESULT CContainerUI::Add(CControlUI* pControl)
	{
		...
		return m_items.Add(pControl) >= 0 ? (0L) : (-1L);
	}

	LRESULT CContainerUI::AddAt(CControlUI* pControl, LONG iIndex)
	{
		if ( pControl == NULL) return (-1L);
		...
		return m_items.InsertAt(iIndex, pControl) >= 0 ? (0L) : (-1L);
	}

	LRESULT CContainerUI::Remove(CControlUI* pControl, BOOL bDoNotDestroy)
	{
		if ( pControl == NULL) return (-1L);
		...
		for( int it = 0; it < m_items.GetSize(); it++ ) {
			if ( static_cast<CControlUI*>(m_items[it]) == pControl ) {
				...
				return m_items.Remove(it) >= 0 ? (0L) : (-1L);
			}
		}
		return (-1L);
	}

	LRESULT CContainerUI::RemoveAt(LONG iIndex, BOOL bDoNotDestroy)
	{
		CControlUI* pControl = GetItemAt(iIndex);
		if (pControl != NULL) {
			return CContainerUI::Remove(pControl, bDoNotDestroy) >= 0 ? (0L) : (-1L);
		}

		return (-1L);
	}

	LRESULT CContainerUI::RemoveAll()
	{
	...
	LONG CContainerUI::GetChildPadding() const
	{
		return m_iChildPadding;
	}

	void CContainerUI::SetChildPadding(LONG iPadding)
	{
	...
	LRESULT CContainerUI::DoEvent(TEventUI& event)
	{
	...
	LONG CContainerUI::FindSelectable(LONG iIndex, BOOL bForward /*= true*/) const
	{
	...
	LRESULT CContainerUI::SetPos(RECT rc, BOOL bNeedInvalidate)
	{
		CControlUI::SetPos(rc, bNeedInvalidate);
		if ( m_items.IsEmpty() ) return (-1L);
	...
	LRESULT CContainerUI::Move(SIZE szOffset, BOOL bNeedInvalidate)
	{
	...
	LRESULT CContainerUI::DoPaint(HDC hDC, const RECT& rcPaint, CControlUI* pStopControl)
	{
		RECT rcTemp = { 0 };
	...
	void CContainerUI::SetFloatPos(LONG iIndex)
	{
	...
	void CContainerUI::ProcessScrollBar(RECT rc, LONG cxRequired, LONG cyRequired)
	{
		if (m_pHorizontalScrollBar != NULL) {
			if ( m_pVerticalScrollBar == NULL ) {
	...
	BOOL CContainerUI::SetSubControlFixedHeight( LPCTSTR pstrSubControlName, LONG cy )
	{
		CControlUI* pSubControl=NULL;
		pSubControl=this->FindSubControl(pstrSubControlName);
		if (pSubControl!=NULL)
	...
	BOOL CContainerUI::SetSubControlFixedWdith(LPCTSTR pstrSubControlName, LONG cx)
	{
		CControlUI* pSubControl=NULL;
		pSubControl=this->FindSubControl(pstrSubControlName);
		if (pSubControl!=NULL)
	...
	LONG CContainerUI::GetSubControlFixedHeight( LPCTSTR pstrSubControlName )
	{
		CControlUI* pSubControl=NULL;
		pSubControl=this->FindSubControl(pstrSubControlName);
		if (pSubControl==NULL)
	...
	LONG CContainerUI::GetSubControlFixedWdith( LPCTSTR pstrSubControlName )
	{
		CControlUI* pSubControl=NULL;
		pSubControl=this->FindSubControl(pstrSubControlName);
		if (pSubControl==NULL)
